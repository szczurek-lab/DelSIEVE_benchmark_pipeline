...
compute.tree.distance <- function(row.data) {
  ...
  if (grepl("sieve", row.data["tool"], ignore.case = TRUE) || grepl("sciphi", row.data["tool"], ignore.case = TRUE)) {
  ...
  is.rooted <- !grepl("cellphy", row.data["tool"], ignore.case = TRUE)
  ...
  if (grepl("sciphi", row.data["tool"], ignore.case = TRUE)) {

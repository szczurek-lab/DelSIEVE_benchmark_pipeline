...
if snakemake.params["fineTuneType"] is None:
    ...
    fine_tune_types = np.repeat(
        snakemake.params["fineTuneType"], len(snakemake.input["inferredTrees"])
    )
else:
    fine_tune_types = []
...
if snakemake.params["dataType"] is None:
    ...
    data_types = np.repeat(
        snakemake.params["dataType"], len(snakemake.input["inferredTrees"])
    )
else:
    data_types = []
    data_type_flag = True
...
for __tool_setup in tool_setups:
    for dataset_name in sorted(snakemake.params["datasetNames"]):
        ...
        inferred_idx, __inferred_trees = get_matched_files(
            snakemake.input["inferredTrees"], __pat
        )
        cnt = np.sum(inferred_idx)

        if cnt == 0:
            print(
                f"Warning: no matches found for {__pat} in {snakemake.input['inferredTrees']}."
            )
        ...
        true_idx, __true_trees = get_matched_files(
            snakemake.input["trueTrees"], f"{NON_WORDS_PATTERN}{dataset_name}$"
        )
        if np.sum(true_idx) == 0 or np.sum(true_idx) > 1:
            raise ValueError(
                f"{dataset_name} should have only a single true tree, but {np.sum(true_idx)} found."
            )
        true_trees_file_names.extend(np.repeat(__true_trees, cnt))

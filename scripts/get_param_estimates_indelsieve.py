...
def parse_params(
    params_file: str, estimate_type: str
) -> tuple[float, float, float, float, float, float, float, float]:
    if os.path.isfile(params_file):
        deletion = insertion = allelic_seq_cov = allelic_seq_var = eff_seq_err_rate = (
            ado
        ) = sc_1 = sc_2 = gamma = pop = np.nan
            ...
            for line in fh:
                ...
                if reach_beginning is True:
                    if not line.strip().startswith("#"):
                        ...
                        if (
                            comp[1].strip() in estimate_type
                            or estimate_type == "mode"
                            and comp[1].strip() not in ["mean", "median"]
                        ):
    ...
    for dataset_name in sorted(snakemake.params["datasetNames"]):
        ...
        for __params in __inferred_params:
                ...
                for _, _, files in os.walk(os.path.dirname(__params), topdown=True):
                    estimate_type_idx = which_patterns(
                        ...
                        [
                            f"(?=.*{i})(?=.*\.vcf$)"
                            for i in vec_pattern(
                                candidate_estimate_types, "[^a-zA-Z0-9]+"
                            )
                        ],
                    )
                    __ret = [
                        candidate_estimate_types[j]
                        for i in estimate_type_idx
                        for j in i
                        if j >= 0
                    ]
            ...
            (
                ...
                gamma,
                pop,
            ) = parse_params(__params, estimate_types[-1])

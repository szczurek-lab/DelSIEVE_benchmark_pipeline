...
cell.names <- read.csv(snakemake@input$cellNames)[[1]]
...
file.list <- read.table(snakemake@input$indelsieveVarCallFiles)
...
estimated.allelic.info <- read.table(paste(dirname(snakemake@input$indelsieveVarCallFiles), estimated.allelic.info.file, sep = "/"))
sieve.cell.names <- read.table(paste(dirname(snakemake@input$indelsieveVarCallFiles), sieve.cell.names.file, sep = "/"))
estimated <- tibble(
...
results <- tibble(
    ...
    doublet_rate = snakemake@params$doubltRate,
    cnv_rate = snakemake@params$cnvRate,

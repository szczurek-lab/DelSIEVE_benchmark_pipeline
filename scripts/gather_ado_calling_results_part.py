...
def _get_content_files(file: str) -> list[str] | None:
...
def _get_cell_names(input: str, is_true: bool) -> pd.DataFrame | None:
...
def _get_loci_info(input: str) -> list[tuple[str, int]] | None:
    if os.path.isfile(input):
        results = []
...
def _get_ado_states(row_num: int, col_num: int, input: str) -> np.ndarray | None:
    if os.path.isfile(input):
...
def _get_properties(
    ...
    _ado_states = _get_ado_states(
        len(_loci_info), joined_cell_names.shape[0], ado_states
    )

    tp = fp = tn = fn = tp_one_ado = fp_one_ado = tn_one_ado = fn_one_ado = (
        tp_two_ados
    ) = fp_two_ados = tn_two_ados = fn_two_ados = t_two_ados_as_no_ado = (
        t_two_ados_as_one_ado
    ) = 0
        ...
        for _line in fh:
            if not _line.startswith("#"):
                ...
                if (_comp[0], int(_comp[1])) in _loci_info:
                    ...
                    for _, row in joined_cell_names.iterrows():
                        if row.is_doublet:
                            continue

                        _true_ado_state_value = int(_comp[2 + row.index_in_true])
                        _ado_state_value = _ado_states[_id, row.index_in_pred]
...
if snakemake.params["fineTuneType"] is None:
    ...
    fine_tune_types = np.repeat(
        snakemake.params["fineTuneType"], len(snakemake.input["allFiles"])
    )
else:
    fine_tune_types = []
...
for __tool_setup in sorted(snakemake.params["toolSetup"]):
    for __data_type in sorted(snakemake.params["dataType"]):
        for dataset_name in sorted(snakemake.params["datasetNames"]):
            ...
            true_ado_states.extend(
                np.repeat(
                    os.path.abspath(
                        snakemake.params["trueAdoStatesPre"] + "." + dataset_name
                    ),
                    cnt,
                )
            )
            ...
            _, __ado_states = get_matched_files(__files, r"\.ado$")
...
ado_collection = pd.DataFrame(
    {
        ...
        "true_positive_single_ado": pd.Series(TP_one_ado),
        "false_positive_single_ado": pd.Series(FP_one_ado),
        "true_negative_single_ado": pd.Series(TN_one_ado),
        "false_negative_single_ado": pd.Series(FN_one_ado),
        "recall_single_ado": pd.Series(recall_one_ado),
        "precision_single_ado": pd.Series(precision_one_ado),
        "f1_score_single_ado": pd.Series(f1_score_one_ado),
        "fall_out_single_ado": pd.Series(fall_out_one_ado),
        "true_positive_locus_ado": pd.Series(TP_two_ados),
        "false_positive_locus_ado": pd.Series(FP_two_ados),
        "true_negative_locus_ado": pd.Series(TN_two_ados),
        "false_negative_locus_ado": pd.Series(FN_two_ados),
        "recall_locus_ado": pd.Series(recall_two_ados),
        "precision_locus_ado": pd.Series(precision_two_ados),
        "f1_score_locus_ado": pd.Series(f1_score_two_ados),
        "fall_out_locus_ado": pd.Series(fall_out_two_ados),
    }
)
